...
public class CyberwareMenuScreen extends Screen {
    ...
    @Override
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {int centerX = width / 2;
        ...
        double mouseAngle = Math.atan2(dy, dx);for (int i = 0; i < parts.size(); i++) {
            ...
            g.pose().pushPose();String statusText = isActive ? "有効化" : "無効化";int textColor = 0xFFFFFFFF;
