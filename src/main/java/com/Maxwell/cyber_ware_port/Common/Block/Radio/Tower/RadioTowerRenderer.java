...
public class RadioTowerRenderer implements BlockEntityRenderer<RadioTowerCoreBlockEntity> {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(CyberWare.MODID, "textures/block/radio_tower_complete.png");public RadioTowerRenderer(BlockEntityRendererProvider.Context context) {
    ...
    public void render(RadioTowerCoreBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBufferSource, int pPackedLight, int pPackedOverlay) {
        ...
        pPoseStack.pushPose();pPoseStack.translate(0.5D, -7.5D, 0.5D);

        pPoseStack.mulPose(Axis.ZP.rotationDegrees(180.0F));pPoseStack.translate(0.0D, 1.0D, 0.0D);VertexConsumer vertexConsumer = pBufferSource.getBuffer(RenderType.entityCutoutNoCull(TEXTURE));
