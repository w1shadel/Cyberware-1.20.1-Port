...
public class AssemblyRecipe implements Recipe<SimpleContainer> {
    ...
    @Override
    public boolean matches(SimpleContainer pContainer, Level pLevel) {return true;
    }
    ...
    @Override
    public RecipeSerializer<?> getSerializer() { return ModRecipes.ASSEMBLY_SERIALIZER.get(); }
    @Override
    public RecipeType<?> getType() { return ModRecipes.ASSEMBLY_TYPE.get(); }
    @SuppressWarnings("removal")
    public static class Serializer implements RecipeSerializer<AssemblyRecipe> {
        ...
        public AssemblyRecipe fromJson(ResourceLocation pRecipeId, JsonObject pJson) {
            ...
            for (int i = 0; i < inputArray.size(); i++) {
                JsonObject entry = inputArray.get(i).getAsJsonObject();Ingredient ing;
