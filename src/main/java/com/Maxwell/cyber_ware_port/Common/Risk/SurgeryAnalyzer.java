...
public class SurgeryAnalyzer {
    ...
    public static SurgeryAlert check(List<Slot> slots, int maxTolerance) {
        ...
        int projectedCost = 0;for (int i = 0; i < RobosurgeonBlockEntity.TOTAL_SLOTS; i++) {
        ...
        int remainingTolerance = maxTolerance - projectedCost;boolean hasRightLeg = hasItemInSlot(slots, RobosurgeonBlockEntity.SLOT_LEGS);     
        ...
        boolean hasLeftArm  = hasItemInSlot(slots, RobosurgeonBlockEntity.SLOT_ARMS + 1);if (!futureParts.contains(BodyPartType.BRAIN)) {
