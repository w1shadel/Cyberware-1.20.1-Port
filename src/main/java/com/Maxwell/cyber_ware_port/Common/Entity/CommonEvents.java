package com.Maxwell.cyber_ware_port.Common.Entity;import com.Maxwell.cyber_ware_port.Common.Entity.Monster.CyberCreeper.CyberCreeperEntity;
...
public class CommonEvents {
    ...
    private static List<Item> CACHED_HIGH_TIER_POOL = null;@SubscribeEvent
    public static void onLivingDrops(LivingDropsEvent event) {
        ...
        RandomSource random = entity.getRandom();LivingEntity attacker = null;
 ...
 i++) {
        ...
        if (attacker != null && attacker.getMainHandItem().getItem() == ModItems.KATANA.get()) {
            ...
            float katanaChance = 0.50f + (looting * 0.10f);if (random.nextFloat() < katanaChance) {
    ...
    private static List<Item> generateMobDropPool(ICyberwareMob cyberMob) {
        List<Item> pool = new ArrayList<>();pool.addAll(CACHED_COMMON_POOL);if (cyberMob.isHighTierMob()) {
    ...
    private static void initDropPools() {
        ...
        CACHED_HIGH_TIER_POOL = new ArrayList<>();List<Item> highTierItems = new ArrayList<>();
...
for (RegistryObject<Item> entry : ModItems.ITEMS.getEntries()) {
            ...
            if (item instanceof CyberwareItem) {
                ResourceLocation id = ForgeRegistries.ITEMS.getKey(item);if (id != null && id.getPath().contains("body_part")) continue;

                if (item == ModItems.CREATIVE_BATTERY.get()) continue;if (highTierItems.contains(item)) {
    ...
    public static void onExplosionStart(ExplosionEvent.Start event) {
        ...
        if (event.getExplosion().getExploder() instanceof CyberCreeperEntity creeper) {
            ...
            event.setCanceled(true);Level level = event.getLevel();

            if (level.isClientSide) return;creeper.setCausingCustomExplosion(true);float baseRadius = creeper.isPowered() ? 6.0F : 3.0F;

            float finalRadius = baseRadius + 1.0F;long time = level.getDayTime() % 24000;
