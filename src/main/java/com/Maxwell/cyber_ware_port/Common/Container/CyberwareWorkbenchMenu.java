...
case 2 -> {
                ...
                case 4 -> { blueprintCurrentPage = value;
 updateSlotPositions();
 }
...
public CyberwareWorkbenchMenu(int pContainerId, Inventory inv, FriendlyByteBuf extraData) {
 ...
 col < 3;
 col++) {
                            Slot slot = new SlotItemHandler(handler, col + row * 3, -10000, -10000);
 ...
 col++) {
    ...
    @Override
    public void setData(int id, int data) {super.setData(id, data);
 ...
 col++) {
    ...
    public int getMaxPages() { return pageData.get(1);
 }
    public int getBlueprintCurrentPage() { return pageData.get(4);
 }
    public int getBlueprintMaxPages() { return pageData.get(5);
 }
    ...
    public ItemStack quickMoveStack(Player playerIn, int pIndex) {
        ...
        int PLAYER_END = totalSlots;if (pIndex < WB_END) {
        ...
        } else if (pIndex >= LIB_START && pIndex < LIB_END) { 
            if (!moveItemStackTo(sourceStack, PLAYER_START, PLAYER_END, true)) return ItemStack.EMPTY;
            ...
            if (sourceStack.getItem() instanceof BlueprintItem) {
                ...
                if (!moveItemStackTo(sourceStack, CyberwareWorkbenchBlockEntity.BLUEPRINT_SLOT, CyberwareWorkbenchBlockEntity.BLUEPRINT_SLOT + 1, false)) {
                    ...
                    if (hasBlueprintLibrary && rightCount > 0) {
                        if (!moveItemStackTo(sourceStack, LIB_START, LIB_END, false)) return ItemStack.EMPTY;
                ...
                if (!moveItemStackTo(sourceStack, CyberwareWorkbenchBlockEntity.INPUT_SLOT, CyberwareWorkbenchBlockEntity.INPUT_SLOT + 1, false)) {
                    if (isExtendedOpen && leftCount > 0) {
                        if (!moveItemStackTo(sourceStack, EXT_START, EXT_END, false)) return ItemStack.EMPTY;

                    }
                }
            } else if (sourceStack.is(net.minecraft.world.item.Items.PAPER)) {
                if (!moveItemStackTo(sourceStack, CyberwareWorkbenchBlockEntity.PAPER_SLOT, CyberwareWorkbenchBlockEntity.PAPER_SLOT + 1, false)) return ItemStack.EMPTY;
