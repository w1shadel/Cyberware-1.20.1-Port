...
public class EmiEngineeringRecipe implements EmiRecipe {
    ...
    public EmiEngineeringRecipe(RecipeHolder<EngineeringRecipe> holder) {
        this.holder = holder;
        this.input = EmiIngredient.of(holder.value().input());
        this.outputs = new ArrayList<>();
        for (EngineeringRecipe.OutputEntry entry : holder.value().outputs()) {
            this.outputs.add(EmiStack.of(entry.stack()));
        }
        ItemStack bpStack = ItemStack.EMPTY;
        if (holder.value().input().getItems().length > 0) {
            bpStack = BlueprintItem.createBlueprintFor(holder.value().input().getItems()[0].getItem());
        }
