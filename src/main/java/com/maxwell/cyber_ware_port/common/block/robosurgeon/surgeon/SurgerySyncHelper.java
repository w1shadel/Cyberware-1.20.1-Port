...
public class SurgerySyncHelper {
    public static boolean updateGhosts(ItemStackHandler body, ItemStacksResourceHandler table) {
        boolean changed = false;
        for (int i = 0; i < table.size(); i++) {
            ItemStack b = body.getStackInSlot(i);
            ItemStack t = table.getResource(i).toStack((int) table.getAmountAsLong(i));
