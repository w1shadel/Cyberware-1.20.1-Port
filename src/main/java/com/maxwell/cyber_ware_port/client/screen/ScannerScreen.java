...
public class ScannerScreen extends AbstractContainerScreen<ScannerMenu> {
    ...
    public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float a) {
        ...
        if (!logLines.isEmpty()) {
            ...
            graphics.text(this.font, line, x + 8, y + 20, logColor, false);
        }
        int barX = x + 4;
        int barY = y + 30;
        graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, barX, barY, 0, 166, 161, 8, 256, 256);
        int maxBarWidth = 161;
        int progressWidth = menu.getScaledProgress(maxBarWidth);
        if (progressWidth > 0) {
            graphics.blit(RenderPipelines.GUI_TEXTURED, TEXTURE, barX, barY, 0, 175, progressWidth, 8, 256, 256);
        }
